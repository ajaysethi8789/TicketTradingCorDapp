...
//}
//
//fun createReport(mockNet: MockNetwork, nodeIssuer: StartedMockNode, nodeDealer: StartedMockNode): FestivalTicketState {
//    val issuer = nodeIssuer.info.legalIdentities.first()
//    val dealer = nodeDealer.info.legalIdentities.first()
//    val report = FestivalTicketState("1.0", FestivalTicketState.ColorScale.D, FestivalTicketState.ClarityScale.VI1, "oval", issuer, dealer)
//
//    val reportFuture = nodeIssuer.startFlow(CreateFestivalTicketFlow(report))
//
//    mockNet.runNetwork()
//
...
//}
//
//fun verifyGradingReport(node: StartedMockNode, report: FestivalTicketState) {
//    val criteria = QueryCriteria.VaultQueryCriteria(
//            status = Vault.StateStatus.UNCONSUMED
//    )
//    val vault = node.services.vaultService.queryBy<ContractState>(FestivalTicketState::class.java, criteria = criteria).states
//
//    assertEquals(1, vault.size)
//
//    val token = vault.first().state.data as FestivalTicketState
//
